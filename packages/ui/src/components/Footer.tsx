...
// Section — the page footer band. A full-width region with the logo (+ optional
// CTA) on a top row, a responsive grid of link columns, and a copyright note.
// Mobile-first: columns stack 2-up on phones and expand to 4 at tablet+.
// Light (surface-secondary) is the default; `dark` paints action-primary with
// white text for use at the bottom of dark pages. Use once per page.

// ── Boltz wordmark SVG (self-contained, mirrors NavBar's BoltzLogo) ────────────
...
const BoltzLogo = ({ className }: { className?: string }) => (
  <svg
    ...
    aria-label="Boltz"
  >
    <g clipPath="url(#footer-boltz-logo-clip)">
    ...
    <defs>
      <clipPath id="footer-boltz-logo-clip">
...
const footerVariants = cva('w-full', {
  variants: {
    background: {
      light: 'bg-surface-secondary text-text-primary',
      dark: 'bg-action-primary text-text-on-dark',
    },
  },
  defaultVariants: { background: 'light' },
});

// Per-element colour maps (multi-element component → typed Record per convention).
const titleColor: Record<NonNullable<FooterProps['background']>, string> = {
  light: 'text-text-muted',
  dark: 'text-white/60',
};

const linkColor: Record<NonNullable<FooterProps['background']>, string> = {
  light: 'text-text-secondary hover:text-text-primary',
  dark: 'text-white/70 hover:text-white',
};

const noteColor: Record<NonNullable<FooterProps['background']>, string> = {
  light: 'text-text-muted',
  dark: 'text-white/60',
};
...
export interface FooterColumn {
  /** Optional column heading. The Boltz footer (Figma 58:379) uses plain link
   *  lists with no titles — omit for that. */
...
export interface FooterProps
  ...
  /** Logo slot — defaults to the Boltz wordmark SVG. */
  logo?: React.ReactNode;
  /** Link columns. Stack 2-up on mobile, 4 across at tablet+. */
  columns: FooterColumn[];
  /** Optional CTA slot, rendered top-right opposite the logo. */
  cta?: React.ReactNode;
...
export const Footer = React.forwardRef<HTMLElement, FooterProps>(
  ({ className, logo, columns, cta, note, background, ...rest }, ref) => {
    const variant = background ?? 'light';
    return (
      <footer ref={ref} className={cn(footerVariants({ background }), className)} {...rest}>
        {/* Layout mirrors Figma 58:379: logo on the left, plain link columns to
            the right (gap-16 vertical lists, no titles), copyright below. */}
        <div className="max-w-container mx-auto px-md tablet:px-40 py-2xl">
          <div className="flex flex-col gap-2xl tablet:flex-row tablet:justify-between">
            <a href="/" className="inline-flex items-center flex-shrink-0 self-start">
              {logo ?? <BoltzLogo />}
            </a>
              ...
              <div className="flex flex-wrap gap-2xl">
                {columns.map((column, i) => (
                  <div key={i} className="flex flex-col gap-md">
                    {column.title && (
                      <p className={cn('text-body-sm', titleColor[variant])}>{column.title}</p>
                    )}
                    <ul className="flex flex-col gap-md">
                      {column.links.map((link, j) => (
                        <li key={`${i}-${j}-${link.label}`}>
                          <a
                            href={link.href}
                            className={cn(
                              'text-body-sm no-underline hover:underline',
          ...
          {note && <p className={cn('mt-2xl text-body-sm', noteColor[variant])}>{note}</p>}
        </div>
      </footer>
    );
  },
